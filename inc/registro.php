...
{ // Aqui dentro validaremos todo y grabaremos en la base de datos.
	...
	if (count($errores)==0)
	{
		...
		// Preparamos la SQL.
		$sql=sprintf("insert into users(nickname,name,surname,password,email,birthday,datecreated) values('%s','%s','%s','%s','%s','%s','%s')",$_POST['nickname'],$_POST['name'],$_POST['surname'], encriptar($_POST['password']),$_POST['email'],cambiarf_mysql($_POST['birthday']),time(),$_SERVER['REMOTE_ADDR'],1);
